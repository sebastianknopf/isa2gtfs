...
```shell
python -m isa2gtfs -i ./input.zip -o ./output.zip [-c ./config.yaml] [-d ivustandard]
```
...
| config.generate_feed_id | Whether to write the column feed_id (unofficial!) in feed_info.txt or not; this column might be used by systems like OpenTripPlanner |
