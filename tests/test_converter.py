...
class ConverterTests(unittest.TestCase):
    ...
    def test_write_txt_file_writes_csv(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            output_file = os.path.join(tmp_dir, 'out.txt')
    ...
    @mock.patch('isa2gtfs.dialect.init51.convert')
    ...
    def test_convert_zip_paths_calls_dialect_and_cleanup(
        ...
        mock_init51_convert: mock.MagicMock,
        mock_listdir: mock.MagicMock,
        mock_remove: mock.MagicMock,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            input_directory = os.path.join(tmp_dir, 'input')
            output_directory = os.path.join(tmp_dir, 'output')
            os.makedirs(input_directory)
            os.makedirs(output_directory)

            input_zip = os.path.join(input_directory, 'input.zip')
            output_zip = os.path.join(output_directory, 'output.zip')

            in_ctx = mock.MagicMock()
            in_zip = mock.MagicMock()
            in_ctx.__enter__.return_value = in_zip

            out_ctx = mock.MagicMock()
            out_zip = mock.MagicMock()
            ...
            converter = IsaGtfsConverter()
            generated_1 = os.path.join(output_directory, 'a.txt')
            generated_2 = os.path.join(output_directory, 'b.txt')
            ...
            mock_init51_convert.assert_called_once_with(converter, input_directory, output_directory)
    ...
    def test_convert_with_unknown_dialect_logs_error(self, mock_log_error: mock.MagicMock) -> None:
        converter = IsaGtfsConverter(dialect='unknown')
